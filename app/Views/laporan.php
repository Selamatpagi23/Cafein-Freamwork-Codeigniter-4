...
<div class="card">
    <!-- <button onclick="printTable()" class="btn btn-success fw-bold">Cetak DATA PENJUALAN</button> -->
    ...
    <div class="card-body">
        <!-- menampilkan data semua LAPORAN  -->
...
<script>
    ...
    function laporanSemua(tanggalMulai, tanggalSelesai) {
        ...
        var tabel = '<table id="tabelLaporan" class="display table table-striped table-hover" ><thead><tr><th>NO</th><th>TANGGAL</th><th>NAMA</th><th>HARGA</th><th>JUMLAH</th><th>TOTAL</th><th>PELANGGAN</th></tr></thead><tbody>'
        ...
        $.ajax({
            ...
            success: function(data) {
                for (let i = 0; i < data.length; i++) {
                    keuntungan = (data[i].harga_akhir * 1)
                    ...
                    tabel += '<td>'+ formatRupiah((data[i].harga_akhir).toString()) +'</td>'
    ...
    function laporanAntrian(tanggalMulai, tanggalSelesai) {
        ...
        $.ajax({
            ...
            success: function(data) {
                for (let i = 0; i < data.length; i++) {
                    totalKeuntungan += data[i].harga_akhir * 1
                    tabel += '<tr>'
                    tabel += '<td>' + (i + 1) + '</td>'
                    tabel += '<td>' + data[i].nama + '</td>'
            ...
            success: function(data) {
                if (data) {
                    for (let i = 0; i < data.length; i++) {
                        ...
                        $("#totalHarga").val(formatRupiah(formatRupiah((data[i].total_bayar).toString())));
                    }
                } else {
